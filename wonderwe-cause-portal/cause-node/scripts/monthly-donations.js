...
function getMonthlyDonationsForThisDay() {


  excuteQuery.queryForAll('SELECT * FROM recurring_gift_tbl WHERE payment_gateway="wepay" AND date_deleted IS NULL AND (timestampdiff(day,date_created,now())>1) and (timestampdiff(day,date_created,now())%30)=0 ORDER BY id DESC; ', [], function(err, result) {
    if (err) {
      console.log(err);
    } else {
      console.log('Total monthly donations:',result.length);
      async.eachSeries(result, function(ele, eachCallback) {
        excuteQuery.queryForAll(sqlQueryMap['getMonthlyDonationStatus'],[ele.id,moment().month(),moment().year()],function(err,result){
          if(err){
            eachCallback(null);
          }else{
            if(!result || !result[0] || (result[0].status != 'payment_recorded' && result[0].status!= "payment_success")){
              ...
              doMonthlyDonation(ele, function(err, result) {
                if(err){
                  callback(err, null);
                }else{
        ...
        if (err) {
          utility.nodeLogs('ERROR',{message:'Error in monthly donations'});
        } else {
...
function doMonthlyDonation(ele, callback) {
  ...
  async.waterfall([
    ...
    function(code, callback) {
      if(ele.status_id){
        excuteQuery.queryForAll(sqlQueryMap['updateMonthlyDonationStatus'], ['payment_', ele.id, month, year], function(err, result) {
            if (err) {
    ...
    function(prev_transaction, callback) {      
      ...
      wp.call('/checkout/create', transactionObj, function(response) {
        ...
        if (responseObj.error) {
          console.log('Error in wepay checkout');
          var errObj = {
            status: 400,
      ...
      excuteQuery.queryForAll(sqlQueryMap['updateMonthlyDonationStatus'], ['payment_recorded', ele.id, month, year], function(err, result) {
        if (err) {
          ...
          excuteQuery.queryForAll(sqlQueryMap['getCodeById'], [ele.code_id], function(err, codeData) {
            ...
            sendEmailtoDonater(donationObj, function(err, reuslt) {
              callback(null, result);
            });
    ...
    if(err){
      utility.nodeLogs('ERROR',{message:'Error in running monthly donations',
        error:err,
        data:ele});
    }else{
...
getMonthlyDonationsForThisDay();

function sendEmailtoDonater(donationObj, callback) {
        ...
        if (err) {
          ...
          if (donationObj.account_type == 'claimed') {
           ...
           // utility.nodeLogs('info', "unclaimed charity");
            ...
            console.log(finalobjectmandril.email);
            utility.mandrillTemplate(finalobjectmandril, function(err, data) {
              if (err) {
                callback(err);
